...
describe('생산과 저장', () => {
    ...
    it('초기 저장 호환 및 손상/미래 버전 거부', () => { const s = initial(0); expect(decodeSave(JSON.stringify({ ...s, version: 1 })).inventory).toEqual({}); expect(() => decodeSave('{')).toThrow(); expect(() => decodeSave(JSON.stringify({ ...s, version: 14 }))).toThrow(); expect(() => decodeSave(JSON.stringify({ ...s, inventory: { wood: -1 } }))).toThrow(); });
