...
describe('왕국 복원 프로젝트', () => {
  ...
  it('v1~v9 저장은 모든 기존 기능과 생산 작업을 유지하고 회수품을 재지급하지 않는다', () => {
    ...
    // 다리·무너진 식당·길드 회관·버려진 밭을 포함해 그 시점에 존재하는 모든 프로젝트가
    // 이미 완료된 것으로 이전된다 — "이미 열려 있던 기능을 잃지 않는다" 원칙이 대장간에만
    // 국한되지 않는다.
  ...
  it('부서진 다리 도입 전(v10) 저장은 대장간 상태를 보존하고 다리는 미시작으로 이전한다', () => {
    ...
    // 새로 추가된 다리·식당·길드 회관·버려진 밭은 자동 완료되지 않고 미시작 상태로 나타난다 — 직접 진행해야 한다.
  ...
  it('v12 저장(대장간+다리+식당 시절)은 세 프로젝트만 검증하고, 그 버전에 없어야 할 구역이 섞여 있으면 거부한다', () => {
    ...
    // v12 그대로는(길드 회관·밭 키가 없는 채) 정상 통과해야 한다.
    expect(decodeSave(JSON.stringify(raw)).version).toBe(14);
  });
  ...
  it('v13 저장(대장간+다리+식당+길드 회관 시절)은 네 프로젝트만 검증하고, 그 버전에 없어야 할 구역이 섞여 있으면 거부한다', () => {
    ...
    // v13 그대로는(밭 키가 없는 채) 정상 통과해야 한다.
    expect(decodeSave(JSON.stringify(raw)).version).toBe(14);
  });

  it('v14 저장은 다섯 프로젝트를 모두 검증하고, 저장 버전에 없어야 할 구역이 섞여 있으면 거부한다', () => {
    const s = initial(0);
  ...
  it('v11 저장(대장간+다리 시절)은 두 프로젝트만 검증하고, 그 버전에 없어야 할 구역이 섞여 있으면 거부한다', () => {
    ...
    // v11 그대로는(식당·길드 회관·밭 키가 없는 채) 정상 통과해야 한다.
    expect(decodeSave(JSON.stringify(raw)).version).toBe(14);
  });
